...
modal_cups <- most_common(coffee_survey$cups)

```

Most participants report drinking `r modal_cups` cups of coffee a day. This is called the mode.
...
There were `r nrow(coffee_survey)` participants. Among male participants, `r inline_text(coffee_table, variable = "employment_status", level = "Employed full-time", column = "stat_1")` were employed full-time.

## Table 2 (Fit a regression and present well-formatted results from the regression)

```{r}
coffee_survey <- coffee_survey |> 
  mutate(high_spender = ifelse(total_spend %in% 
                                 c("$60-$80", "$80-$100", ">$100"), 1, 0))

logistic_model <- glm(high_spender ~ cups + age + gender, 
                      data = coffee_survey, 
                      family = binomial())
...
Compared to participants who reported drinking 1 cup per day, those who drank 2 cups had more than twice the odds of being a high spender (OR = 2.26, 95% CI: 1.82–2.80, *p* \< 0.001).

## Figure 1

```{r}

barplot(table(coffee_survey$where_drink),
        main = "Figure 1: Where People Typically Drink Coffee",
        xlab = "Location",
...
Most people report drinking coffee at home.
